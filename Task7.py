...
def load_xml_file(file_path):
    try:
        ...
        print(f"Niepoprawna składnia pliku XML. Błąd: {e}")
        return None
...
def save_xml_file(file_path, root):
    try:
...
def verify_file_syntax():
    file_path = input("Podaj ścieżkę do pliku JSON, YAML lub XML: ")
    if file_path.endswith(".json"):
        ...
        print("Nieobsługiwany format pliku. Obsługiwane formaty: .json, .yml, .yaml, .xml")
        return
    if data:
        ...
        if output_file_path.endswith(".json"):
            ...
            print("Nieobsługiwany format pliku wyjściowego. Obsługiwane formaty: .json, .yml, .yaml, .xml")
